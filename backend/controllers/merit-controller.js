...
export const meritController = {
    getAllMerits: async (req, res) => {
        try{
            console.log('Getting all merits');
            ...
            const meritWithUsers = await Promise.all(merits.map(async (merit) => {
                const usersSnapshot = await usersCollection.where('userId', '==', merit.userId).get();
